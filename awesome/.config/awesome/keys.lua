...
-- Awesome
  ...
  awful.key({ mod }, 'e', function() awful.util.spawn('rofi -show drun -show-icons -theme themes/material/3/default') end),
...
keys.clientkeys = gears.table.join(
  awful.key({ mod }, 'c', function(c) c:kill() end),
  awful.key({ mod }, 'm', function(c) c.minimized = true end),
  awful.key({ mod }, 'f', function(c)
    c.fullscreen = not c.fullscreen;
    c:raise()
  end),
  awful.key({ mod }, 'v', function() awful.client.floating.toggle() end)
)
